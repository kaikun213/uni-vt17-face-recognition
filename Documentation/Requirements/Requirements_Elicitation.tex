...
\begin{enumerate}[leftmargin=0.8in]
		\item \label{UAMLogging} The UAM shall support the logging of users
			\stepcounter{enumii}
			\begin{enumerate}
				\item The initial window of the UAM shall contain a field for user name, a field for a password and button labeled "login". The password field shall be a "secret" field, which does not display what the user types.
				\item When a user presses the login button, the UAM shall send a request to the URM to login the user.
			\end{enumerate}
		\item If and only if the logging of a user is successful (see \ref{UAMLogging}) the UAM shall display the USW and the user shall be able to use the functionality of the application.
		\item When the user presses the "GET PN" Button in the USW the UAM shall open the camera.
		\item \label{UAMCAM} If the camera returns a picture the UAM shall send a request to the URM with the taken picture.
		\item \label{UAMRet} The UAM shall display responses from the URM and provide a button to return to the USW.
...
\begin{enumerate}[leftmargin=0.8in]
		\item Upon successful login request from the UAM the URM shall produce the USW as specified in \ref{UAMLogging}.
		\item The URM shall be able to decrypt all incoming requests and encrypt all outgoing responses.
		\item The URM only replies to authenticated requests.
		\item The URM shall be able to classify a picture it gets from a request and possibly match it with a corresponded PN.
		\begin{enumerate}
			\item If the URM receives a request it shall forward a request to the EFR with the received picture.
			\item The URM shall be able to receive a response from the EFR and process it.
		\end{enumerate}  
...
\begin{enumerate}[leftmargin=0.8in]
		...
		\item The ARM only replies to authenticated requests.
		\item \label{ARMRequest}The ARM shall validate and execute all requests from an authenticated ACM.
		\item The ARM shall be able to make requests to the Database.
		\item The ARM shall be able to make requests to the EFR.
		\item The ARM shall be able to receive and process responses of the EFR.
\end{enumerate}
