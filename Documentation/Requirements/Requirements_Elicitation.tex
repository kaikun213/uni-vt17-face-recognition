...
\begin{enumerate}
		...
		\item \label{UCMGETsend} When the user presses the "GET PN" Button in the USW the UCM shall open the camera.
		\item If the camera returns a picture the UCM shall send a request to the USM with the taken picture.
		\item \label{UCMGETret} The UCM shall display responses from the USM and provide a button to return to the USW.
...
\begin{enumerate}
		\item The ASM shall be able to authenticate Admins.
		...
		\item The ASM shall validate and execute all requests from an authenticated ACM.
		\item \label{ASMGET} The ASM shall be able to make requests to the Database.
...
\begin{tabular}{|p{3.5cm}|p{11.5cm}|} \hline
    ...
    \textbf{Primary flow} & 
    \textbf{1)} The User provides an image of a person's face (see \ref{UCMGETsend}) \\&
    \textbf{2)} The User receives a matching PN (see \ref{UCMGETret}) \\&
...
\begin{tabular}{|p{3.5cm}|p{11.5cm}|} \hline
    ...
    \textbf{Description} &  The admin should be able to get a list of all the PNs from the ASN (see \ref{ASMGET}).
